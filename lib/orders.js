 ...
 *
 * PERBAIKAN AUDIT: sebelumnya baris ini adalah `if (!db || !productId)
 * return true;` — kalau Firebase Admin belum terkonfigurasi di backend
 * (env var FIREBASE_KEY kosong/invalid di Vercel), fungsi ini DIAM-DIAM
 * menganggap stok selalu tersedia, TANPA melempar error sama sekali.
 * Karena api/create-payment.js sekarang fail-closed pada saat error
 * (lihat catatan di file itu), memperbaiki catch-nya saja tidak cukup -
 * kasus `!db` ini tidak pernah masuk ke catch karena tidak pernah throw.
 * Sekarang dipisah:
 *   - !productId -> produk ini memang tidak pakai fitur stok (unlimited
 *     by design), tetap return true seperti sebelumnya.
 *   - !db -> Firebase Admin belum terkonfigurasi. Kalau productId ADA
 *     (artinya produk ini memang pakai fitur stok), kita TIDAK BOLEH
 *     diam-diam menganggap stok tersedia - lempar error supaya
 *     create-payment.js menghentikan proses (STOP, jangan generate QRIS)
 *     alih-alih diam-diam menjual stok yang tidak bisa diverifikasi.
 */
async function hasAvailableStock(productId, packageName) {
  ...
  if (!db) {
    throw new Error(
      "Firebase Admin belum terkonfigurasi di backend (FIREBASE_KEY kosong/invalid) - stok tidak bisa diverifikasi."
    );
  }
